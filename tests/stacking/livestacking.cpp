...
TEST_CASE("(LiveStacking) Cancel during processing", "[LiveStacking]")
{
    class Listener : public LiveStackingListener
    {
    public:
        void progressNotification(const live_stacking_infos_t& infos) override
        {
            ...
            REQUIRE(infos.lightFrames.nbStacking <= infos.lightFrames.nbValid);
            REQUIRE(infos.lightFrames.nbStacked <= infos.lightFrames.nbValid);

            if (!cancelled && (infos.lightFrames.nbStacking == 3))
            {
                cancelled = true;
    ...
    public:
        void progressNotification(const live_stacking_infos_t& infos) override
        {
            ...
            if (infos.lightFrames.nbStacked == 3)
            {
                ...
                REQUIRE(infos.lightFrames.nbStacking == 0);
...
TEST_CASE("(LiveStacking) Add dark frame during processing", "[LiveStacking]")
{
    ...
    public:
        ...
        {
            if (darkFrameAdded)
                REQUIRE(((infos.nbDarkFrames == 3) || (infos.nbDarkFrames == 2)));
            else
...
TEST_CASE("(LiveStacking) Add light frame during processing", "[LiveStacking]")
{
    ...
    public:
        void progressNotification(const live_stacking_infos_t& infos) override
        {
            ...
            if (lightFrameAdded)
                REQUIRE(((infos.lightFrames.nb == 3) || (infos.lightFrames.nb == 2)));
            else
